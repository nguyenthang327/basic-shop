...
@section('content')
    ...
    <form name="product" action="{{ url("/backend/product/update/$product->id") }}" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <label for="product_price">Khuyến mãi %:</label>
            <input type="text" name="product_sale" style="width: 100px" class="form-control" id="product_sale" value="{{ $product->product_sale }}">
        </div>
